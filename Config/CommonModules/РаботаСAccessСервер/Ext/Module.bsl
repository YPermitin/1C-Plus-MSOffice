...
Функция ВыгрузитьРезультатКомпоновки(КаталогВыгрузки, СхемаКомпоновкиДанных, Настройки, ИмяТаблицы = Неопределено, ОграниченияВыгрузкиКаждойБазы = Неопределено) Экспорт
	...
	Если НЕ ДоступнаВыгрузкаВариантаОтчета(Настройки) Тогда
		ВызватьИсключение
			"Текущие настройки отчета не позволяют выполнить безопасную выгрузку в Excel!
			|
			|На текущий момент поддерживается выгрузка только простых вариантов отчета
			|без группировок и дополнительного форматирования.";
	КонецЕсли;
...
Функция ДоступнаВыгрузкаВариантаОтчета(НастройкиКомпоновщика)
	
	ДоступнаВыгрузка = Истина;	
	
	МассивЭлементовСтруктурыНастроек = Новый Массив;
	Для Каждого Эл Из НастройкиКомпоновщика.Структура Цикл
		Если Эл.Использование = Истина Тогда
			МассивЭлементовСтруктурыНастроек.Добавить(Эл);		
		КонецЕсли;
	КонецЦикла;
	Если МассивЭлементовСтруктурыНастроек.Количество() = 1 Тогда
		ОсновнойЭлемент = МассивЭлементовСтруктурыНастроек.Получить(0);	
		Если ТипЗнч(ОсновнойЭлемент) = Тип("ГруппировкаКомпоновкиДанных") Тогда
			Если ОсновнойЭлемент.Структура.Количество() > 0 Тогда
				ДоступнаВыгрузка = Ложь;
			КонецЕсли;
		ИначеЕсли ТипЗнч(ОсновнойЭлемент) = Тип("ТаблицаКомпоновкиДанных") Тогда
			Если ОсновнойЭлемент.Строки.Количество() <> 1 Тогда
				ДоступнаВыгрузка = Ложь;
			КонецЕсли;
		Иначе
			ДоступнаВыгрузка = Ложь;	
